...
function indexOfRegExp(chaine, cherche) {

    //Récupérer la version sans entitées html du texte
    chaine = html_entity_decode(chaine, '');

    //Enlever les accents
    cherche = removeDiacritics(cherche);
    chaine = removeDiacritics(chaine);

    //Échapper les caractères spéciaux dans l'expression régulière
    cherche = cherche.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, "\\$&");
    cherche = cherche.replace(/\$/g, '$$$$');

    var regExp = new RegExp(cherche, 'i');
    return chaine.match(regExp);
}