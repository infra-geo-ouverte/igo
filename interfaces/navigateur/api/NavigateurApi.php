...
class NavigateurApi extends \Phalcon\Mvc\Micro {
    ...
    function couche($coucheId){
        ...
        foreach ($arrayCoucheId as $key => $value) {
            ...
            $permission = $this->obtenirPermission($couche->id);
        ...
        $this->response->setContentType('application/json; charset=UTF-8')->sendHeaders();
    ...
    function contexteCode($contexteCode){
        $contexte = $this->obtenirContexteParCode($contexteCode);
    ...
    function contexte($contexteId){
        $contexte = $this->obtenirContexte($contexteId);
        $this->obtenirInfoContexte($contexte);
    }
    ...
    */
                    ...
                    if($authentificationModule === null){
                        ...
                        foreach($couches as $couche){
                            ...
                            foreach($coucheContexte as $igoVueContexteCoucheNavigateur ){
                                $permission = $this->obtenirPermission($igoVueContexteCoucheNavigateur->couche_id);
                                if($permission !== null && $permission->est_lecture){
                                    $estPermis = true;
                                    break;
                                }
    ...
    function proxyNavigateur($service){
        ...
        if(isset($_cle)){
            ...
            if($cleM == "POST"){
                $paramsPost['_client_IP'] = NavigateurApi::ObtenirAdresseIP($_SERVER);
                $paramsPost['cle'] = $cle;
            } else {
                $paramsGet['_client_IP'] = NavigateurApi::ObtenirAdresseIP($_SERVER);
                $paramsGet['cle'] = $cle;
            }
        }
        ...
        $this->proxyRequestNavigateur($url, $paramsPost, $method, $options);
    }
    ...
    */
    function obtenirContexteParCode($contexteCode) 
    {
        ...
        if (!$igoContexte) {
            return $this->envoyerResponse(404, "Not Found", "Le contexte '{$contexteCode}' n'existe pas!");
        }
        return $igoContexte;
    }
    ...
    */
    ...
    {
        $this->response->setStatusCode($statusCode, $titre);
        $error = new stdClass();
    ...
    */
    function obtenirAuthentificationModule() 
    {
        ...
        if (!$this->utilisateurActuelEstAuthentifie() && !$this->utilisateurActuelEstAnonyme()) {
            return $this->envoyerResponse(401, "Unauthorized", "Vous n'êtes pas authentifié!");
        }
    ...
    */
    ...
    {
        //TODO Utiliser un accesseur de info_utilisateur ex : ... info_utilisateur->getEstAuthentifie()
        $session = $this->getDI()->getSession();
        if (!$session->has("info_utilisateur")) {
            return false;
        }
    ...
    */
    ...
    {
        if (is_null($this->getDI()->getConfig()->database)) {
            return null;
        }
    ...
    * @param ??? $contexte
    */
    function obtenirInfoContexte($contexte) 
    {
        ...
        $array = array();
        $avertissements = array();
        $debug = $this->getDI()->get("config")->application->debug;
    ...
    * @return
    */
    static function ObtenirAdresseIP($serveur) 
    {
  ...
  function proxyRequestNavigateur($url, $data, $method, $options) {
        ...
        $session_name = $this->getDI()->getSession()->getName();
    ...
    function configuration($configuration){
        ...
        if(!file_exists ($xmlPath) && !curl_url_exists($xmlPath)){
            return $this->envoyerResponse(404, "Not Found", "La configuration '{$configuration}' n'existe pas!");
        }
        ...
        if($encoding === "json"){
            ...
            if ($element->getName() === "navigateur" ){
                ...
                foreach($result as $couche){
                    ...
                    //Vérifier l'access
                    $permission = $this->obtenirPermission($igoVueCouche->id);
                ...
                return $this->envoyerResponse(404, "Not Found", "L'élément racine du fichier de configuration doit se nommer 'navigateur' !");
            }
        }else{
            return $this->envoyerResponse(404, "Not Found", "L'encodage '{$encoding}' n'est pas supporté!");
        }
    }
}