 ...
 * @requires widgets/tree/WMSBrowserRootNode.js
 */
...
GeoExt.ux.tree.WMSBrowserTreePanel = Ext.extend(Ext.tree.TreePanel, {
    ...
    addLayerToPreview: function(layer) {
        ...
        if (!layerPreview) {
            ...
            //if(Host.wmsBrowserProxy) 
            layerPreview.url = Igo.Aide.utiliserProxy(layerPreview.url);
    ...
    addLayer: function() {
        ...
        if (!this.root.hasChildNodes()) {
            ...
            if (this.wmsbrowser.isLayerNameValid()) {
                ...
                /*host_apps = Igo.Aide.obtenirHoteMSP();

                TODO: remettre ce bout de code en place
                //ajout  : pour supporter les appels EGL en https des WMS du MSP, puis changer l'url lors de la connexion en https, Nicolas 28 janv 2011.
                //V�rifier que l'h�te est s�curis�
                if(host_apps.indexOf("https://") > -1){

                        url_layers = newLayer.url;
                        switch(host_apps){

                                case "https://egl.msp.gouv.qc.ca/":
                                        url_layers = url_layers.replace("http://geoegl.msp.gouv.qc.ca/","https://egl.msp.gouv.qc.ca/");
                                        url_layers = url_layers.replace("http://pregeoegl.msp.gouv.qc.ca/","https://egl.msp.gouv.qc.ca/");
                                        break;
                                case "https://preegl.msp.gouv.qc.ca/":
                                        url_layers = url_layers.replace("http://geoegl.msp.gouv.qc.ca/","https://preegl.msp.gouv.qc.ca/");
                                        url_layers = url_layers.replace("http://pregeoegl.msp.gouv.qc.ca/","https://preegl.msp.gouv.qc.ca/");
                                        break;
                                case "https://geoegl.msp.gouv.qc.ca/":
                                        url_layers = url_layers.replace("http://geoegl.msp.gouv.qc.ca/","https://geoegl.msp.gouv.qc.ca/");
                                        url_layers = url_layers.replace("http://pregeoegl.msp.gouv.qc.ca/","https://geoegl.msp.gouv.qc.ca/");
                                        break;
                        ...
                        newLayer.url = url_layers;
                } 
                */
           
		
/*if(Host.wmsBrowserProxy) newLayer.url = Igo.Aide.obtenirProxyWMS()+ escape(newLayer.url);*/
                var addLayer = this.wmsbrowser.fireEvent('beforelayeradded',
                                                         {'layer': newLayer});

                if (addLayer !== false) {
                    var igoCouche = new Igo.Couches.WMS({layerOL:newLayer});
                    Igo.Aide.obtenirNavigateur().carte.gestionCouches.ajouterCouche(igoCouche);
