...
<xsl:template name="nl2br">
  ...
  <xsl:variable name="label" select="document('./xsl/labels.xml')"/>
  <xsl:variable name="value" select="document('./xsl/codelists.xml')"/>
  ...
  <xsl:template name="content">
    <h1>
		<span class="contentImg">
			<img src="{{base}}/inc/images/icon_populaire.png" width="44" height="44"/>
		</span>
      <span>
	  ...
	  <!-- CONDITION ACRIGÉO POUR L'AFFICHAGE -->
		...
		<xsl:when test="gmd:identificationInfo/gmd:MD_DataIdentification/gmd:descriptiveKeywords/gmd:MD_Keywords/gmd:keyword/gco:CharacterString = 'ACRIGéo'">
			...
			<center>
			<img src="{{base}}/inc/images/acrigeo.JPG" width="95" height="50"/> 
			...
			<br/><br/>
			<span><a href="mailto:ACRIGEONETWORK@msp.gouv.qc.ca?Subject=ACRIGeoNetwork"><img src="{{base}}/inc/images/icon_nous_joindre.png"/><font size="3">Joindre l'ACRIgéo</font></a></span>
			</center>
    ...
    <p>
        <xsl:call-template name="nl2br">
            <xsl:with-param name="string" select="gmd:identificationInfo/gmd:MD_DataIdentification/gmd:abstract/gco:CharacterString"/>
        </xsl:call-template>
    </p>
  ...
  <p>
    <xsl:call-template name="nl2br">
      <xsl:with-param name="string"  select="gmd:identificationInfo/gmd:MD_DataIdentification/gmd:purpose/gco:CharacterString"/>
    </xsl:call-template>
  </p>
  ...
  <!--Raccourci-->  
    ...
    <table border="1">
	  ...
	  <!-- AFFICHAGE DE LA LICENCE PAR DÉFAUT - DONNÉES OUVERTES -->
		<xsl:otherwise>
          <a href="http://www.donnees.gouv.qc.ca/?node=/licence">Licence sur l’utilisation des données ouvertes</a>
		</xsl:otherwise>
		</xsl:choose>
	<!-- FIN DE LA CONDITION ACRIGÉO POUR L'AFFICHAGE -->
			...
			<xsl:text disable-output-escaping="no"> </xsl:text>				
				...
				<xsl:choose>
					<xsl:when test="gmd:CI_OnlineResource/gmd:name/gco:CharacterString != ''">
						<a href="{gmd:CI_OnlineResource/gmd:linkage/gmd:URL}" target="_new">
						 <img alt="télécharger" src="{{base}}/inc/images/download.png"/> &#160; <xsl:value-of select="gmd:CI_OnlineResource/gmd:name/gco:CharacterString"/>			
						</a>	
					</xsl:when>
					<xsl:otherwise>
						<a href="{gmd:CI_OnlineResource/gmd:linkage/gmd:URL}" target="_new">
						 <img alt="télécharger" src="{{base}}/inc/images/download.png"/> &#160; Télécharger le fichier			
						</a>	
					</xsl:otherwise>
				</xsl:choose>
