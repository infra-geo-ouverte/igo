...
if(isset($config['html2canvas'])) {
    $config = $config['html2canvas'];

    if(isset($config['domainesAutorises'])) {
        $domainsWhitelist = $config['domainesAutorises'];
    }
}
...
*/
$maxExec = 100;
ini_set('max_execution_time', $maxExec);
...
if(isset($_GET['image'])) {
    ...
    $imageName = basename($requestedImage);
...
function isUrlAllowed($domainsWhitelist, $url) {
    if(!$domainsWhitelist) {
        return false;
    }
...
} else if(isUrlAllowed($domainsWhitelist, $_GET['url']) == false) {
    $response = array('error' => 'Url domain is not allowed');
            ...
            if (CROSS_DOMAIN === 1) {
                ...
                echo $param_callback, '(',
                    JsonEncodeString(
                        ...
                        $dir_name. '/' . $_REQUEST["_url"] . '?image=' . basename($locationFile)
                    ),
                ');';
            }
            exit;
